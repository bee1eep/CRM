...
void SingletonDataBase::openDB(){
    ...
    db.setDatabaseName("C:/Users/User/Desktop/CRM/back/CRMb.sqlite");

    if (db.open()) {
    qDebug()<<"-_---------";
    QSqlQuery qquery(db);

    qquery.exec("SELECT *  FROM user");
    qDebug()<<"-_---------";
    while (qquery.next()) {
             QString name = qquery.value(0).toString();
             QString salary = qquery.value(1).toString();
             qDebug() << name << salary;
         }
          return;
    }
